    ...
    Contudo, ao contrário dos estudos laboratoriais, onde os dados apenas interessam no final da recolha, num cenário real o objetivo é responder às mudanças do estado de cada indivíduo à medida que estas ocorrem. Por esta razão, a tomada de decisões exige a atualização dos dados em tempo real (ou com latência reduzida), sendo por isso necessário adotar uma abordagem em \textit{streaming}.

    \section{Objetivos}\label{sec:objectives}

    O objetivo deste trabalho é mostrar que é possível conjugar uma solução técnica baseadana monitorização simultânea de vários utilizadores durante o uso normal do computador, recorrendo apenas ao uso de periféricos normalmente presentes - interfaces do próprio computador e/ou dispositivos \textit{wearable off-the-shelf}.
