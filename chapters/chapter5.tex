...
A estrutura da mensagem, serializada em formato JSON (como visto na Listagem~\ref{code:example_message}), foi desenhada para ser concisa, apresentando um payload médio de 165 bytes (sem contar com \textit{overhead} TCP/IP). Esta dimensão é composta pela soma dos campos identificadores (45 bytes para o \ac{UUID}, 31 bytes para o nome do sensor e 30 bytes para o timestamp), pelo valor da medição (30 bytes) e pela sintaxe estrutural inerente ao formato JSON (29 bytes).
...
\begin{lstlisting}[language=json, caption={Exemplo da estrutura de uma mensagem JSON}, label={code:example_message}]
{
...
\midrule
Teclado & Eventos de digitação & $5$\tnote{1} \\
WearOS (\ac{HRV}, estimado a partir de BPM) & Monitorização cardíaca & $1$\tnote{2} \\
Webcam & Processamento de vídeo/metadados & $10$\tnote{3} \\ 
\midrule
...
Ou seja, podemos estimar que cada cliente envia cerca de 16 (cerca de 2640) mensagens por segundo para estes sensores (sendo que, mesmo se cada )
