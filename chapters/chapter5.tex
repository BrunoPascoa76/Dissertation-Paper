...
\begin{enumerate}
    ...
    \item O valor é lido pelo Kafka Connect e inserido na base de dados (Figuras~\ref{fig:example_4}~e~\ref{fig:example_45}).
\end{enumerate}
...
\begin{figure}[H]
     ...
     \begin{subfigure}[t]{0.19\textwidth}
         ...
         \caption{Receção da mensagem no \textit{broker} local}
         \label{fig:example_1}
     \end{subfigure}
     \hfill
     \begin{subfigure}[t]{0.19\textwidth}
     ...
     \begin{subfigure}[t]{0.19\textwidth}
         \centering
         \includegraphics[width=\linewidth]{figs/example/grafana}
         \caption{Dashboard do Grafana}
         \label{fig:example_5}
     \end{subfigure}     
...
WearOS\tnote{2} (\ac{BPM}) & Monitorização cardíaca & $1$ \\
