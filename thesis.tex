...
\TitlePT{Ecosistema de análise de sinais com base em streaming}
...
\Abstract{palavras-chave}{programação,streaming,Kafka,sensores,processamento de dados}
         {resumo}{\selectlanguage{portuguese}A monitorização contínua de sistemas baseados em múltiplos sensores requer uma arquitetura capaz de suportar a aquisição e processamento contínuo dos dados. Abordagens tradicionais apresentam limitações de escalabilidade e resiliência, não sendo para isso apropriadas para a recolha de dados a longo prazo.\\Esta dissertação propõe uma arquitetura escalável e modular para aquisição e monitorização de dados de sensores, baseada numa abordagem baseada em streaming. A solução utiliza o Apache Kafka como camada de intermediação entre produtores e consumidores. ao InfluxDB para o armazenamento dos dados indexados pelo tempo e uma aplicação desktop baseada em Python para a monitorização e controlo dos sensores}
...
\appendix
\pagestyle{plain}
\include{chapters/appendix-requirements}
\include{chapters/appendix-architecture}
\end{document}
