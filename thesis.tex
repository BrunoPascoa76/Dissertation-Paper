...
\Abstract{palavras-chave}{programação, streaming, modular, Kafka, InfluxDB, MQTT, sensores, processamento de dados}
         {resumo}{\selectlanguage{portuguese}A monitorização contínua do estado psicofisiológico e comportamental de utilizadores em ambientes de computação depende da recolha eficaz de dados provenientes de múltiplas fontes, como dispositivos de entrada padrão (rato e teclado) ou sensores \textit{wearable}. No entanto, a integração e processamento em tempo real destes fluxos de dados apresentam desafios críticos em termos de resiliência, integridade e escalabilidade, limitando a viabilidade de estudos remotos ou que envolvam a recolhade dados a longo prazo.
         \newline Esta dissertação propõe o desenho e implementação de uma infraestrutura modular, extensível e agnóstica ao sensor, concebida para suportar estudos longitudinais centrados na interação do utilizador com o computador. O sistema foca-se na recolha contínua e processamento em \textit{streaming} de dados provenientes de sensores presentes no próprio computador (como teclado ou câmara), bem como sensores \textit{off-the-shelf} facilmente conectáveis (como \textit{smartwatches}), tirando partido do protocolo MQTT para captação de dados localmente, do ecossistema Apache Kafka para orquestração e transporte de dados e do InfluxDB como base de dados de série temporal para persistência.
         \newline De modo a garantir a validação do sistema, foi também proposto a recolha de dados de câmara (direção do olhar), teclado e frequência cardíaca (via \textit{smartwatch}), bem como monitorização via Grafana em tempo real e a estimativa dos requisitos de hardware necessários.}
...
\Abstract{keywords}{programming, streaming, modular, Kafka, InfluxDB, MQTT, sensors, data processing}
          {abstract}{\selectlanguage{english}The continuous monitoring of a user's psychophysiological and behavioural state while using a computer depends on the effective gathering of data from multiple sources, like standard input devices (mouse and keyboard) or wearable sensors. However, the integration and real-time processing of these data flows presents critical challenges in terms of resilience, integrity and scalability, limiting the viability of remote or long-term data gathering studies.
          \newline This dissertation proposes the design and implementation of a modular, extensibvle and data-agnostic architecture, made to suport longitudinal studies centered on the user's interaction with the computer. The system focuses on the continuous stream-based gathering and processing of data from sensors both integrated in the computer itself (like camera or keyboard), as well as off-the-shelf sensors (like smartwatches), utilizing the MQTT protocol for local data gathering, the Apache Kafka echosystem for orchestration and InfluxDB for persistence.
          \newline Aditionally, in order to validate the system, it was also proposed to implement the gathering of data from a webcam (gaze tracking), keyboard and heart rate (via smartwatch), as well as monitoring the gathered data in real-time using Grafana and then estimated the expected system load in order to measure hardware requirements.}
