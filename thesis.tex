...
\Abstract{palavras-chave}{programação, streaming, modular, Kafka, InfluxDB, MQTT, sensores, processamento de dados}
         {resumo}{\selectlanguage{portuguese}Um dos maiores desafios na monitorização contínua de múltiplos indivíduos em condições não laboratoriais é o custo e intrusividade dos sensores tradicionais. Em resposta a estes desafios, e tirando proveito do uso generalizado do computador para fins profissionais e académicos, o uso dos periféricos do próprio computador ou dispositivos \textit{wearable} de uso comum tem ganho popularidade como uma alternativa mais acessível e menos intrusiva para a monitorização contínua de utilizadores.
          \newline Contudo, apesar de já existirem soluções comerciais de monitorização, a ausência de uma solução que permita adaptar as fontes de dados e as métricas delas extraídas motivou-me a criar uma \textit{framework} focada na extensibilidade e flexibilidade.
          \newline Esta dissertação apresenta o desenho e implementação de uma \textit{framework} extensível de monitorização contínua de múltiplos indivíduos baseada em \textit{streaming}, capaz de integrar dinamicamente novos sensores e métricas a partir de periféricos e dispositivos \textit{wearable off-the-shelf}.}
...
\Abstract{keywords}{programming, streaming, modular, Kafka, InfluxDB, MQTT, sensors, data processing}
          ...
          \newline However, despite commercial  monitoring solutions already existing, the lack of a solution that allows adding different data sources or metrics based on the user's needs has motivated me to create a framework focused on extensibility and flexibility.
          \newline This dissertation presents the design and development of an extensible streaming-based framework capable of monitoring multiple users simultaneously by dynamically integrating new sensors and metrics from computer peripherals or off-the-shelf wearable devices.}
...
\include{chapters/appendix-fault-tolerance}
\pagestyle{plain}
\end{document}
